...
{	
	pos = [self convertToISO:ccp(pos.x, pos.y)];
	...
	CCTMXLayer *layerColl = [tilemap layerNamed:@"Layer coll"];
...
{	
	...
	if (p1i.x < p2i.x)
	{
		while (i < p2i.x)
		{
			...
			gid0 = [layer0 tileGIDAt:ccp(i, j)];
			if (kTagTileBlock == gid || 0 == gid0) return actions; // cliccato su posizione nulla
